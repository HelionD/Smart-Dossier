...
export default function KanbanScreen() {
  ...
  return (
    <View style={styles.root}>
      ...
      {/* Kanban columns */}
      {isLoading ? (
        <ActivityIndicator color={Colors.secondary} style={{ marginTop: 48 }} size="large" />
      ) : (
        <ScrollView
          horizontal
            ...
            return (
              <View key={phase} style={styles.column}>
                ...
                <View style={styles.columnHeader}>
                  <Text style={[
                    styles.columnLabel,
                    isBottleneck && styles.columnLabelWarn,
                  ...
                  <View style={[
                    styles.countBadge,
                    isBottleneck && styles.countBadgeWarn,
                    isCompleted && styles.countBadgeDone,
                  ]}>
                    <Text style={[
                      styles.countText,
                      isBottleneck && styles.countTextWarn,
                ...
                {isCompleted ? (
                  <View style={styles.completedCard}>
                    ...
                    <Text style={styles.completedTitle}>{phaseCases.length} cases completed</Text>
                    <TouchableOpacity onPress={() => router.push({ pathname: '/(clerk)/cases', params: { phase: '7' } })}>
...
function KanbanCard({ caseItem }: { caseItem: Case }) {
  ...
  return (
    <TouchableOpacity
      style={[styles.card, caseItem.is_blocked && styles.cardBlocked]}
    ...
    >
      ...
      <Text style={styles.cardTitle} numberOfLines={2}>{caseItem.title}</Text>
      <View style={styles.cardBottom}>
        <View style={styles.cardDays}>
          <Text style={styles.clockIcon}>{'\u25F7'}</Text>
          <Text style={[styles.daysText, caseItem.is_blocked && styles.daysTextUrgent]}>
            {caseItem.days_in_phase} {caseItem.days_in_phase === 1 ? 'day' : 'days'}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerSub: { ...Typography.bodySm, color: Colors.onSurfaceVariant, marginTop: 2, fontSize: 12 },
  blockedBadge: {
  ...
  blockedBadgeText: { ...Typography.labelCaps, color: Colors.onErrorContainer, fontSize: 10 },
  trackScroll: { flex: 1 },
  trackContent: {
  ...
  cardBlocked: { borderColor: Colors.error, borderWidth: 2 },
  cardTop: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  daysText: { ...Typography.labelCaps, color: Colors.onSurfaceVariant, fontSize: 10 },
  ...
  cardAvatarText: { fontFamily: 'HankenGrotesk_700Bold', fontSize: 10, color: Colors.onSurfaceVariant },
  completedCard: {
