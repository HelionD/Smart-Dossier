...
import { dosjet, ai, dokumentet } from '../api/services';
import {
...
type Tab = 'permbledhje' | 'dokumentet' | 'workflow' | 'ai';

export default function CaseDetailScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const [tab, setTab] = useState<Tab>('permbledhje');
  const [advanceNotes, setAdvanceNotes] = useState('');
  const qc = useQueryClient();

  const { data: dosja, isLoading } = useQuery({
    queryKey: ['dosja', id],
    queryFn: () => dosjet.get(id),
  });

  const { data: history = [] } = useQuery({
    queryKey: ['history', id],
    queryFn: () => dosjet.phaseHistory(id),
    enabled: tab === 'workflow',
  });

  const { data: docs = [] } = useQuery({
    queryKey: ['docs', id],
    queryFn: () => dokumentet.list(id),
    enabled: tab === 'dokumentet',
  });
  ...
  const advanceMutation = useMutation({
    mutationFn: (newPhase: number) =>
      dosjet.advancePhase(id, { new_phase: newPhase, notes: advanceNotes }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['dosja', id] });
  ...
  const canAdvance = dosja.current_phase < 7;
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.phaseStrip}>
        {[1,2,3,4,5,6,7].map((p) => (
          <View
            ...
            style={[
              styles.phaseNode,
              p < dosja.current_phase && styles.phaseNodeDone,
              p === dosja.current_phase && styles.phaseNodeActive,
              BOTTLENECK_PHASES.includes(p) && p === dosja.current_phase && styles.phaseNodeBottleneck,
            ]}
          >
            <Text style={[
              styles.phaseNodeText,
              p <= dosja.current_phase && styles.phaseNodeTextDone,
            ]}>
              {p < dosja.current_phase ? '✓' : String(p)}
            </Text>
          </View>
        ))}
      </View>
      <Text style={styles.phaseCurrentLabel}>
        Faza {dosja.current_phase}: {PHASE_LABELS[dosja.current_phase]} · {dosja.days_in_phase} ditë
      </Text>

      {/* Tabs */}
      <View style={styles.tabBar}>
        {(['permbledhje', 'dokumentet', 'workflow', 'ai'] as Tab[]).map((t) => (
          <TouchableOpacity
            key={t}
          ...
          >
            <Text style={[styles.tabText, tab === t && styles.tabTextActive]}>
              {t === 'permbledhje' ? 'Përmbledhje' :
               t === 'dokumentet' ? 'Dokumentet' :
               t === 'workflow' ? 'Procesi' : 'AI ✦'}
            </Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <ScrollView style={styles.tabContent} contentContainerStyle={{ padding: Spacing.marginPage, gap: 14 }}>
        ...
        {tab === 'permbledhje' && (
          <>
            <InfoCard label="Aplikuesi" value={dosja.owner_name ?? '—'} />
            <InfoCard label="ID Prona" value={dosja.property_id ?? '—'} />
            <InfoCard label="Zona" value={dosja.zone ?? '—'} />
            <InfoCard label="Kategoria e të ardhurave" value={dosja.income_bracket ?? '—'} />
            <InfoCard label="Statusi" value={dosja.status.toUpperCase()} highlight />
            <InfoCard label="Krijuar më" value={new Date(dosja.created_at).toLocaleDateString('sq-AL')} />
          </>
        )}
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                ...
                if (!result.canceled && result.assets[0]) {
                  ...
                  await dokumentet.upload(id, { uri: file.uri, name: file.name, type: file.mimeType ?? 'application/pdf' });
                  qc.invalidateQueries({ queryKey: ['docs', id] });
                }
              }}
            >
              <Text style={styles.uploadBtnText}>+ Ngarko dokument</Text>
            </TouchableOpacity>
            {docs.map((doc) => (
              <View key={doc.id} style={styles.docCard}>
                ...
                {doc.extracted_data && (
                  <View style={styles.extractedFields}>
                    <Text style={styles.extractedLabel}>Të dhëna të ekstraktuara</Text>
                    ...
                    {!doc.confirmed && (
                      <TouchableOpacity
                        ...
                        onPress={() => dokumentet.confirmExtraction(id, doc.extracted_data!)}
                      >
                        <Text style={styles.confirmBtnText}>Konfirmo ekstraktimin</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                )}
          ...
          <>
            {history.map((log, i) => (
              <View key={log.id} style={styles.logRow}>
                ...
                <View style={styles.logContent}>
                  ...
                  <Text style={styles.logDate}>{new Date(log.entered_at).toLocaleDateString('sq-AL')}</Text>
            ...
            {canAdvance && (
              <View style={styles.advanceCard}>
                <Text style={styles.advanceTitle}>Kalo në fazën {dosja.current_phase + 1}</Text>
                <Text style={styles.advanceSub}>{PHASE_DESCRIPTIONS[dosja.current_phase + 1]}</Text>
                <TextInput
                  style={styles.notesInput}
                  placeholder="Shënime (fakultative)..."
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    Alert.alert(
                      'Konfirmo kalimin',
                      `Kaloni nga faza ${dosja.current_phase} në fazën ${dosja.current_phase + 1}?`,
                      [
                        { text: 'Anulo', style: 'cancel' },
                        { text: 'Konfirmo', onPress: () => advanceMutation.mutate(dosja.current_phase + 1) },
                      ]
                    );
                  }}
                ...
                >
                  {advanceMutation.isPending
                    ...
                    : <Text style={styles.advanceBtnText}>Kalo në fazën {dosja.current_phase + 1} →</Text>
                  }
                </TouchableOpacity>
              </View>
            )}
        ...
        {tab === 'ai' && (
          <>
            <TouchableOpacity style={styles.aiBtn} onPress={() => fetchSummary()}>
              <Text style={styles.aiBtnText}>✦  Analizo dosjen me AI</Text>
            </TouchableOpacity>
            {summaryLoading && (
              <View style={styles.skeletonBox}>
                <ActivityIndicator color={Colors.secondary} />
                <Text style={styles.skeletonText}>Duke analizuar…</Text>
              </View>
            )}
            {summary && !summaryLoading && (
              <View style={styles.summaryCard}>
                <Text style={styles.summaryLabel}>ANALIZË AI</Text>
                <Text style={styles.summaryText}>{summary}</Text>
              </View>
            )}
            {BOTTLENECK_PHASES.includes(dosja.current_phase) && (
              <View style={styles.bottleneckWarning}>
                <Text style={styles.bottleneckTitle}>⚠  Fazë me rrezik vonese të lartë</Text>
                <Text style={styles.bottleneckBody}>
                  Faza {dosja.current_phase} historikisht ka vonesa prej{' '}
                  {dosja.current_phase === 3 ? '2–4 javësh' : '4–8 javësh'} për shkak të procesit manual me ASHK.
                </Text>
              </View>
            )}
          </>
