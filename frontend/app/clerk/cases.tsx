...
export default function CasesScreen() {
  ...
  const { data: cases = [], isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['dosjet', selectedPhase],
    queryFn: () => dosjet.list(selectedPhase ?? undefined),
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        <Text style={styles.headerLabel}>Të gjitha dosjet</Text>
        <Text style={styles.headerCount}>{filtered.length} raste</Text>
      </View>
      ...
      <View style={styles.searchWrap}>
        <TextInput
          ...
          placeholder="Kërko kod, emër, aplikues…"
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={styles.filterContent}>
        ...
        >
          <Text style={[styles.filterChipText, selectedPhase === null && styles.filterChipTextActive]}>Të gjitha</Text>
        </TouchableOpacity>
        {[1, 2, 3, 4, 5, 6, 7].map(p => (
          <TouchableOpacity
            key={p}
      ...
      >
        {isLoading ? (
          ...
          <View style={styles.empty}>
            <Text style={styles.emptyText}>Asnjë dosje e gjetur</Text>
          </View>
        ) : (
...
function CaseItem({ dosja }: { dosja: Dosja }) {
